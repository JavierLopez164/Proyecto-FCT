...
public interface ComidaRepositorio extends JpaRepository<Comida, ComidaPK> {
	...
	@Query("SELECT c.comida.comidaPK.nComida, COUNT(c) AS veces FROM Pedido p JOIN p.items c GROUP BY c.comida.comidaPK.nComida ORDER BY veces DESC")
	List<Object[]> top5ComidasMasPedidas();

	@Query("SELECT c.comida.comidaPK.nComida, COUNT(c) AS veces FROM Pedido p JOIN p.items c WHERE c.comida.comidaPK.nRestaurante = ?1 GROUP BY c.comida.comidaPK.nComida ORDER BY veces DESC")
